...
export class AudiofileUpload extends React.Component {
  ...
  async addAudioUrl(fileUrl) {
    const { studentId, userId, student_stage, change, columnIndex } =
      this.props;
    try {
      if (fileUrl) {
        const url =
        ...
        if (response.data.sucess) {
          ...
          this.props.enqueueSnackbar("successfully uploaded audio file!", {
            variant: "success",
          });
          ...
          this.props.enqueueSnackbar("Something is went wrong!", {
            variant: "error",
          });
        }
      }
