...
export class UIData extends PureComponent {
  ...
  async componentDidMount() {
    ...
    if (params.id) {
      ...
      this.EditRowHandler({ EachRowData, page, screenSize, updatedTable, value });
      this.setState({
        value: value
      });
    }
  ...
  EditRowHandler = ({ EachRowData, page, screenSize, updatedTable, value }) => {
    ...
    return (
      <Fragment>
        ...
        <Grid item xs={12} style={{ padding: 10 }}>
          {isEditRow
            ? <PartnersPaginationPriority data={ListOfData} onClick={this.EditRowHandler} PageShowing={this.state.ShowingPage} StylingForRow={this.state.StylingForRow} EditedData={this.state.EditableTableRowValues} isEditRow={this.state.isEditRow} TableData={TableData} NameLIst={this.state.name} search={this.state.value} />
            : <PartnersPaginationPriority data={ListOfData} onClick={this.AddRowHandler} PageShowing={this.state.ShowingPage} StylingForRow={this.state.StylingForRow} isAddRow={this.state.isAddRow} TableData={TableData} NameLIst={this.state.name} search={this.state.value} />
          }
        </Grid>
      </Fragment>
    );
